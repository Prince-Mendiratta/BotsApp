    ...
    async handle(client, chat, BotsApp, args) {
        ...
        try { 
            ...
            if (isNaN(args[0]) || args[0][0] === '+') {
                client.sendMessage(BotsApp.from, "*Enter a valid Number like '9712345678'.* For Non-Indian number, enter in international format without any spaces or special characters.\n```For example, 17842534789.```", MessageType.text);
                return;
            }
            ...
            if (response[number + '@c.us'] == 408) {
                ...
                client.sendMessage(BotsApp.from, "*The number you're trying to add is already present in the group.*", MessageType.text);
                return;
            }
            client.sendMessage(BotsApp.from, ("```" + number + " was added successfuly.!```"), MessageType.text)

        } catch (err) {
            console.log(err)
            if (err.status == 400) {
                client.sendMessage(BotsApp.from, "*The number you're trying to add is not available on WhatsApp. Please check the number again.*", MessageType.text)
                return;
            }
