...
module.exports = {
    ...
    async handle(client, chat, BotsApp, args) {
        ...
        if(args.length == 0){
            var enable = await Greetings.checkSettings(BotsApp.chatId, "goodbye");
            var Msg = await Greetings.getMessage(BotsApp.chatId, "goodbye");
            try{
            if(enable === false || enable === undefined){
                client.sendMessage(BotsApp.chatId, GOODBYE.SET_GOODBYE_FIRST, MessageType.text); 
                return;
            }
            else if(enable === "OFF"){
                console.log("Greetings are off");
                client.sendMessage(BotsApp.chatId,  GOODBYE.CURRENTLY_DISABLED, MessageType.text);
                client.sendMessage(BotsApp.chatId, Msg.message, MessageType.text); 
                return;
            }

            client.sendMessage(BotsApp.chatId,  GOODBYE.CURRENTLY_ENABLED, MessageType.text);
            client.sendMessage(BotsApp.chatId, Msg.message, MessageType.text); 
            }
            ...
            if(args[0]==="OFF"){
                ...
                client.sendMessage(BotsApp.chatId,  GOODBYE.GREETINGS_UNENABLED, MessageType.text);
                return;     
                }
            if(args[0]==="ON"){
                switched = args[0];
                await Greetings.changeSettings(BotsApp.chatId, switched);
                client.sendMessage(BotsApp.chatId,  GOODBYE.GREETINGS_ENABLED, MessageType.text); 
                return;    
                }
            if(args[0]=== "delete"){
                ...
                await client.sendMessage(BotsApp.chatId, GOODBYE.GOODBYE_DELETED, MessageType.text); 
            ...
            if(Msg === false || Msg === undefined){
               ...
               await client.sendMessage(BotsApp.chatId, GOODBYE.GOODBYE_UPDATED, MessageType.text); 
