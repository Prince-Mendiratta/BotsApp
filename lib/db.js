const data = {
  general: {
    ...
    SUDO_PERMISSION:
        "```🌀Hi, BotsApp this side. I noticed you were trying to use my commands in``` *{BotsApp.groupName}*```. However, currently I'm running in {worktype} mode and only the owner and sudo users can use my command``` *{commandName}* ```.\n\n💠If you'd like, you can add BotsApp to your account, in a SUPER EASY way on our website. Link given below! ❤‍🔥```\n\n🔗https://botsapp.com\n🔗https://github.com/DTU-DCODER/BotsApp",
    ERROR_TEMPALTE:
        "```Looks like something went wrong. Need not worry. Here are some logs since when the bot was not responding as expected.```\n```---------```\n```🧐 Command:``` *{commandName}*\n```😎 From Me?:``` *{fromMe}*\n```🗣️ Was a reply?:``` *{isReply}*\n```👥 In a group?``` *{isGroup}*\n```📥 In Inbox?``` *{isPm}*\n```📸 Command with image?``` *{isImage}*\n```🕺🏻 Is Bot group admin?``` *{isBotGroupAdmin}*\n```📈 Was Sender group admin?``` *{isSenderGroupAdmin}*\n```🫂 Was sender sudo?``` *{isSenderSudo}*\n```⚠️ Error:``` \n*[ERROR] BaileysError: Unexpected status in \'action\': Bad Request(400)*\n```---------```\n_To figure out what exactly went wrong, please report/raise the issue on our support chat at_ https://gruplink",
    SUCCESSFUL_CONNECTION: "BotsApp successfuly integrated. Bot is currently working in {worktype} mode. For more information regarding Bot working and permissions check out: \n🔗https://github.com/DTU-DCODER/BotsApp."
  },
  add: {
